        ...
        <div class="content-page">
            ...
            <form method="post" enctype="multipart/form-data" class="w-50 p-3">
                <div class="mb-3">
                    ...
                    <input type="text" value="<?= Input::post('name', '') ?>" class="form-control  <?= !empty($errors['name']) ? 'border-error' : '' ?>" name="name" id="name" placeholder="Name">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control <?= !empty($errors['category_id']) ? 'border-error' : '' ?>" name="category_id" id="category">
                        ...
                        <?php foreach ($categories as $category): ?>
                            <option <?= $category->id == Input::post('category_id', '') ? 'selected' : '' ?> value="<?= $category->id ?>"><?= $category->name ?></option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" value="<?= Input::post('price', '') ?>" class="form-control <?= !empty($errors['price']) ? 'border-error' : '' ?>" name="price" id="price" placeholder="Price">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control" name="status" id="status">
                        <option value="1" <?= Input::post('status', 1) == 1 ? 'selected' : '' ?>>Active</option>
                        <option value="0" <?= Input::post('status', 1) == 0 ? 'selected' : '' ?>>Inactive</option>
                    </select>
                </div>
                <div class="mb-3">
                    <label for="note" class="form-label">Note</label>
                    <textarea type="text" class="form-control <?= !empty($errors['note']) ? 'border-error' : '' ?>" name="note" id="note" placeholder="Note"><?= Input::post('note', '') ?></textarea>
